...
int main(int argc, char **argv) {
    {
        ...
        ASSERT(sizeof(vec) == (5 + 1) * sizeof(int), "Fill in the correct value.");
        // ASSERT(sizeof(vec) == (5 + 1) * sizeof(int), sizeof(vec));
        int ans[]{1, 2, 3, 4, 5};
        ...
        // ISSUE: 在某些标准库实现中，std::vector 的迭代器可能是一个包装器（例如 __wrap_iter<int*>），它可能不直接转换为裸指针类型。
        // ISSUE: (1) 【不推荐，不要操作迭代器的指针】可以使用迭代器的 operator* 来获取指向元素的裸指针
        // ISSUE: (2) 【推荐】使用 std::vector 的 data() 方法来获取指向向量第一个元素的裸指针
    }
    {
        std::vector<double> vec{1, 2, 3, 4, 5};
        {
            ASSERT(vec.size() == 5, "Fill in the correct value.");
            ASSERT(sizeof(vec) == 24, "Fill in the correct value.");// ISSUE: 当前编译器下 sizeof(std::vector<any-typename>) == 24
        ...
        }
        {
            ...
            ASSERT(sizeof(vec) == 24, sizeof(vec));
            vec.pop_back();
            ASSERT(vec.size() == 5, "Fill in the correct value.");
    ...
    }
    {
        ...
        ASSERT(sizeof(vec) == 24, "Fill in the correct value.");
        {
            ...
            ASSERT(vec.capacity() == 48, "Fill in a correct identifier.");// ISSUE: 依然是48，不影响capacity
        }
        {
            vec.reserve(256);
            ASSERT(vec.size() == 16, "Fill in the correct value.");
