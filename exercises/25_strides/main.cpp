...
std::vector<udim> strides(std::vector<udim> const &shape) {
    ...
    for (auto it = shape.rbegin(); it != shape.rend() - 1; ++it) {
        ...
        //     strides[shape.size() - 1] = 1;
        // } else {
        //     strides[shape.size() - 1 - (shape.rbegin() - it)] = strides[shape.size() - (shape.rebegin() - it)] * *it;
        // }
        strides[shape.size() - 2 - (it - shape.rbegin())] = strides[shape.size() - 1 - (it - shape.rbegin())] * *it;
    }
    return strides;
}
